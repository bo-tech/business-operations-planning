...
- [ ] Add applications into ~business-operations~ and use in ~dev-env~
  - [ ] ~cloudnative-pg~ may be an good start, does not need cluster specific
    customization
    - use an app from ~business-operations~ without tweaking it
  - [ ] ~echoserver~ is as simple as it gets
    - use an app and have cluster specific configuration applied
  - [ ] ~hajimari~ uses a few variables out of the cluster settings
    - use an app and have cluster specific configuration applied
