...
- [X] Write first pytest test: boot VM, assert SSH connection works
  - not needed anymore
- [ ] run existing ~k0s-nix~ tests to verify it all works
- [ ] Decide where the test code lives (~demo-ops~, separate repo, or ~business-operations~)
- [ ] review and cleanup below tasks
- [ ] Run recreate-machines playbook
- [ ] Run bootstrap playbook, assert k0s node is Ready and Cilium is running
  - has a challenge: Needs container images
- [ ] Document how to trigger test runs
