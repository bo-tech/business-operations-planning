...
- [X] Test case to bring up a single machine cluster
  - [X] Scaffold test case, "hello world" like check
  - [X] k0s is starting
  - [X] containers starting in the interactive runner
    - needs DHCP on the slirp interface of qemu to work
  - [X] containers are starting
    - Challenge: Needs the images, no network access in a sandboxed environment.
  - [X] Ansible based bootstrap, e.g. Cilium and OpenEBS are installed and working
    - Also depends on container images
  - [X] Ansible based machine creation
    - This would run ~nixos-anywhere~ into the test machine, tricky.
    - Skipped the ~nixos-anywhere~ step for now, then the rest can be tested
      without issues.
- [X] Document how to trigger test runs with examples
- [X] review - Build NixOS installer ISO with test SSH key baked in
  - unclear if still needed currently
  - skip, no need at the moment
...
Results:
- Basic testing based on the NixOS testing framework built in.

Details:
- Cross platform support is very useful. E.g. run from macbook into linux,
  execute locally or remote.
- Currently only with disabled sandbox, so that images can be fetched from the
  network.
- The ~-rebuild~ flag for re-running tests on remote builders needs a
  workaround because of lacking support.

Not yet covered in testing:
- ~nixos-anywhere~ deployment path not yet tested automatically.
